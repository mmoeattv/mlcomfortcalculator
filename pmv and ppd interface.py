...
SCALER_PATH = r"scaler_X.pkl"

@st.cache_resource
def load_models_and_scaler():
    pmv_m, ppd_m, scaler = None, None, None
    for label, path in [("PMV model", PMV_PATH),
                        ("PPD model", PPD_PATH),
                        ("Scaler",    SCALER_PATH)]:
...
def get_predictions(wall_w, depth, orient, wwr, month_str):
    ...
    # Raw features — same column order as training: [Month, WallWidth, Depth, Orientation, WWR]
    ...
    pred_pmv = pmv_model.predict(scaled_features)[0]
    pred_ppd = ppd_model.predict(scaled_features)[0]
...
def build_room_figure(wall_width, room_depth, wwr, height=3.0):
    ...
    def edge(xs, ys, zs, color='#888888', width=4):
        return go.Scatter3d(x=xs, y=ys, z=zs, mode='lines',
                            line=dict(color=color, width=width), showlegend=False)

    fig.add_trace(edge([wx0,wx1,wx1,wx0,wx0],[0,0,0,0,0],[wz0,wz0,wz1,wz1,wz0],
...
with col3:
    ...
    if current_pmv is not None:
        ...
        st.warning("⚠️ Models not loaded. Please run train_models.py first to generate the 3 required .pkl files.")
